--[[
    Modified by Jared (jdf2) to allow for wireless modem support.
    Simply adds the WPP library and replaces the peripheral API with that.
]]--
...
local openedReadFile = fs.open("reactorcontrol", "r")
local oldSourceCode = openedReadFile.readAll()
openedReadFile.close()
 
local openedWriteFile = fs.open("reactorcontrol", "w")
openedWriteFile.write("os.loadAPI(\"wpp\")\nperipheral=wpp" .. oldSourceCode)
openedWriteFile.close()
 
_G["wppNetworkId"] = "network-id"
